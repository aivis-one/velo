...
Triggered by velo update on commit $NEW_COMMIT" || {
    ...
    doctor)
        ...
        echo "--- nginx ---"
        echo -e "${YELLOW}NOT CHECKED -- /etc/nginx/sites-available/velo has no detector yet (tracked separately).${NC}"
        echo ""

        echo "Checked: vite.env keys, backend/.env keys. NOT checked: nginx config."
        if [ "$VITE_DRIFT" -eq 0 ] && [ "$BACKEND_DRIFT" -eq 0 ]; then
            echo -e "${GREEN}✓ 0 drift found in the checks that ran${NC}"
            exit 0
        else
