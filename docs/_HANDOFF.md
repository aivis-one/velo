...
Имя спринта взять из INDEX.md (раздел `Sprint reference`). Сейчас это **Sprint 2.5** (standalone, Sprint 2 closed 2026-05-18):

`D:\02_Projects\velo\docs\05_roadmap\sprint-02.5.md`

Самое важное:
- Раздел **Scope** — three parallel tracks A / B / C
- Раздел **Tasks** — T2.5.X со ссылками на ROADMAP §5.5
- Раздел **Daily log** — хронология сессий (пока пуст — sprint только открыт 2026-05-18)
- Раздел **Gate results** — per-block / per-bundle / per-file gates

Предыдущий sprint-02.md теперь read-only reference (closed); читай его если нужна история по Phase 3 styleguide + Phase 4 Onboarding 8.
...
**Сейчас Sprint 2 Phase 4 — Onboarding 8 ✅ закрыт (2026-05-18). Sprint 2 P0 mockups ВРЕМЕННО приостановлены.**

**Активная фаза:** Sprint 2.5 — DS Completion (block-by-block) + Foundation Transfer (CBSHOME). См. `ROADMAP.md` §5.5.

**Почему Sprint 2.5 вставлен:** Sprint 2 Phase 4 Dashboard 9 попытка показала что reverse-engineering из PNG приводит к 3× rework (alert pills + glass halo + bottom-nav каждый делали по 3 раза). Методология обновлена до v1.3 — теперь §7.0 Block-Harvest-First mandatory. Sprint 2.5 применяет это правило ко всем оставшимся SACRED блокам ДО старта Sprint 3 mass production.

### Entry point для новой сессии (Sprint 2.5 — Track A первый блок)

**Текущая задача:** T2.5.1 — DS completion для **Dashboard 9 block** (Figma SACRED root `541:6648`, 9 frames).

**Что делать (per Methodology §7.0):**

**Phase A — Harvest** (single Figma pass):
1. `use_figma` Plugin API. File key `F7PD5isLfLdyc0q1Bd5n5c`. Walk all 9 frames depth-deep.
2. Sample exact tokens per element: fills (RGB + alpha), strokes (color + weight), cornerRadius, effects (DROP_SHADOW + BACKGROUND_BLUR), typography (size + family/style + letterSpacing + lineHeight).
3. Export all icon GROUP nodes as SVG via `exportAsync({format:'SVG'})`. Chunk by 20KB cap if needed.
4. Document gaps vs current DS state (`variables.css` + `COMPONENTS-CATALOG.md` + `ASSETS-INDEX.md`).
...
2. SVG icons → `02_design-system/assets/icons/` (canonical names `icon-{component}-{variant}.svg`)
3. `02_design-system/assets/ASSETS-INDEX.md` — добавить строки с Figma node provenance
4. `02_design-system/COMPONENTS-CATALOG.md` — full per-component profile entries (Class + Variants + States, When-to-use + Anti-patterns, Tokens consumed + Related, Provenance + Status)
5. **`02_design-system/styleguide/velo-design-system.html`** — визуализация в tabs Tokens / Components / Patterns. БЕЗ ЭТОГО блок НЕ DS-complete.

**Phase C — Mockup rebuild** (T2.5.2):
- Текущий `_dashboard-flow.html` skeleton — **🗑 superseded**. Пересобираем ground-up на DS-complete state.
- Структура combined viewer 9 колонок (PNG-эталон + HTML на DS tokens) идентична `_onboarding-flow.html`.

### Что НЕ делать (антипаттерны из v1.3)

- ❌ **AP-P-6** — не reverse-engineering из PNG-screenshots. PNG = только side-by-side validation, не extraction.
- ❌ **AP-P-7** — не добавлять token в `variables.css` без визуализации в `velo-design-system.html` тем же edit pass.
- ❌ Не плодить custom CSS-классы без `Grep` по `COMPONENTS-CATALOG.md` (Before-naming check — см. ниже).

### Параллельный Track B (Claude Code, не Cowork)

Foundation transfer из `D:\02_Projects\cbshome\frontend\src\`. Reference: `06_project-inputs/CBSHOME-PATTERNS-FOR-VELO.md`. 16 готовых patterns + 18 UI Kit components + 7 layout shells. Apply с rename `C*` → `V*` + token prefix `--bg-*` → `--velo-bg-*`. Не блокирует Track A — параллельный поток.
...
Current entry point (2026-05-18): Sprint 2.5 (standalone) — Tracks A (DS completion) + B (CBSHOME transfer) + C (Documentation English translation).
Active sprint file: 05_roadmap/sprint-02.5.md. Sprint 2 closed 2026-05-18.
Track A first task: T2.5.1 — Dashboard 9 DS harvest into 5 deliverables (tokens + assets + ASSETS-INDEX + COMPONENTS-CATALOG + styleguide.html visualisation).
Track B: CBSHOME patterns transfer per 06_project-inputs/CBSHOME-PATTERNS-FOR-VELO.md (Claude Code).
Track C: Documentation English translation pass (4 sub-phases Conv-1..Conv-4).
Methodology v1.4 rules active: Before-naming check (catalog) · Block-Harvest-First §7.0 · DS Visualisation §6.8 · English-default P6 · AP-P-6 PNG reverse-engineering · AP-P-7 variables.css without styleguide · AP-P-8 Russian narrative in process docs.
v1.4 _HANDOFF changes: English rule added to rules of behavior (with exception table), Шаг 4 reference updated to sprint-02.5.md, startup prompt gains one-line English-rule reminder.
