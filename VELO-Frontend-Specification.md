...
**ОТКРЫТО (operator-ruling):** `RoleSwitchSection` («Режим тестировщика» + кнопка превью заявки) после деплоя станет видим в ПРОДЕ верифицированным мастерам/админам — прежний прод-гейт был флагом. Решить: переименовать/урезать секцию для прода или принять как фичу. Также известный узкий эдж: CLI-демоут админа ПОСЛЕ его свитча оставляет stale `home_role`-маркер (фикс — одна строка в Zod's `set_role.py`, его файл).
...
| TD-FE-E15-ONBOARDING-FLAG | 🧪 | `master_onboarding_completed` не персистится до E15 | ⬜ |
...
TEST-only инструменты (role-switch / повтор онбординга / превью заявки `ui.previewApplyFlow`) —
прод-недостижимы (`RoleSwitchSection` рендерится лишь на TEST), это намеренные тест-аффордансы,
не долг. Полные файл-привязки — Фронтовый Кодекс §10.
