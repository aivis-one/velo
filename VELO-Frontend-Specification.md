...
> **Примечание (2026-06):** переключение режима реализовано — у Мастера/Админа в
> профиле есть точка входа в пользовательский интерфейс; для тестеров на TEST
> доступен ролевой свитч (`ROLE_SWITCH_ENABLED`, only-TEST: смена собственной роли
> user/master/admin по seeded `allowed_roles`).
...
- Все TEST-only инструменты (role-switch, повтор онбординга, превью заявки) — **прод-недостижимы**: единственная точка входа `RoleSwitchSection` рендерится лишь при непустом `allowedRoles` (= `ROLE_SWITCH_ENABLED` на TEST). Сигналы session-only, не персистятся.
