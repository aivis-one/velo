<!--
  VELO Frontend — FeedbacksCategoryView (S2-S3 SPEEDRUN MEGA-2 §C39)
  Category-filtered Diary route: only feedbacks.
-->
...
const grouped = computed<Group[]>(() => {
  const rows: Row[] = (diary.feedbacks as FeedbackResponse[]).map((f) => ({
    key: `f-${f.id}`,
    title: 'Feedback',
...
onMounted(async () => {
  ...
  loading.value = true
  try {
    await diary.fetchFeedbacks()
  } catch {
    toast.error('Не удалось загрузить feedbacks')
  } finally {
    loading.value = false
  }
})
